...
class carte_consulaire:
	...
	def initialise(self,pdf,personne):
		...
		pdf.setFontSize(6)
		pdf.drawString((self.x + 10.1)*cm, (self.y + 5.4)*cm, 'AMBASSADE EN FRANCE')
		...
		textobject.setTextOrigin((self.x + 10.0)*cm, (self.y + 2)*cm)
		...
		pdf.drawImage(personne.photo, (self.x + 10.1)*cm, (self.y + 2.3)*cm, width=2.6*cm, height = 2.6*cm, preserveAspectRatio=True)
		...
		pdf.setFillColorRGB(0,0,0)
		pdf.setFontSize(11)
		pdf.drawString((self.x - DEC + self.COL[3])*cm, (self.y + (self.PRL - 9*self.LIG))*cm, 'Pierre GAZAGNE')
		pdf.drawString((self.x - DEC + self.COL[3])*cm, (self.y + (self.PRL - 10*self.LIG))*cm, 'Consul à LYON')
		...
		pdf.drawImage(self.tampon_rouge,(self.x + self.COL[3] - 0.5)*cm, (self.y - 0.3)*cm, width = 1.9*cm, preserveAspectRatio=True,mask="auto")
